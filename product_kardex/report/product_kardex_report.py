...
class product_kardex_report(report_sxw.rml_parse):
        ...
        for move in stock_move_obj.browse(self.cr, self.uid, move_ids_hasta):
            if move.location_id.id != move.location_dest_id.id:
                res = {
                        ...
                        'n_guia': move.picking_id.name or move.name,
                            }
                ...
                if move.location_id.id != locat_id[0] and move.location_dest_id.id == locat_id[0]:
                    ...
                    if move.location_id.usage == 'inventory' and move.location_dest_id.usage == 'internal':
                        res.update({'concept_move': 'INVENTARIO INICIAL' })
    ...
    def _get_start_date(self, data):
        if data.get('form', False) and data['form'].get('fecha_desde', False):
            d = datetime.strptime(data['form']['fecha_desde'], '%Y-%m-%d')
            d1= datetime.strftime(d, "%d/%m/%Y")
            #logger.error("INNNNNN111111111: %r", d1)
            return d1
        return ''
    
    def _get_end_date(self, data):
        if data.get('form', False) and data['form'].get('fecha_hasta', False):
            d = datetime.strptime(data['form']['fecha_hasta'], '%Y-%m-%d')
            d1=datetime.strftime(d, "%d/%m/%Y")
            return d1
        return ''
