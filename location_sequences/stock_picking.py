...
class stock_picking(osv.osv):
    ...
    def do_partial(self, cr, uid, ids, partial_datas, context=None): # PARA ENTRGAS PARCIALES
        ...
        for pick in self.browse(cr, uid, ids, context=context):
            ...
            for move in pick.move_lines:
                ...
                #_logger.error("PARCIALLL: %r", round(partial_qty[move.id],6))
                if move.product_qty == round(partial_qty[move.id],6):
                    complete.append(move)
                elif move.product_qty > round(partial_qty[move.id],6):
                    too_few.append(move)
                else:
    ...
    #AGREGADO PARA EVITAR QUE PASE DIRECTAMENE A ESTADO LISTO PARA ENVIO EL PICKING DE SALIDA
    def action_assign(self, cr, uid, ids, *args):
        ...
        for pick in self.browse(cr, uid, ids):
            ...
            res = self.pool.get('stock.move').action_assign(cr, uid, move_ids)
            mov_totales_ids = [x.id for x in pick.move_lines]
            mov_reser_ids = [x.id for x in pick.move_lines if x.state == 'confirmed']
            if len(mov_reser_ids) == res or len(mov_totales_ids) == res:
                wf_service.trg_write(uid, 'stock.picking', pick.id, cr)
        return True  
...
class stock_move(osv.osv):
	...
	'''#Comentado para evitar que el picking pase a estado LISTO PARA SER ENVIADO cuando algunos productos no tengan stock
        if count:
...
class sale_order(osv.osv):
    ...
    def _create_pickings_and_procurements(self, cr, uid, order, order_lines, picking_id=False, context=None):
        ...
        if picking_id:
            ...
            else:#COMENTADO POR QUE CAUA CONFUSION
                picking_obj.action_assign(cr, uid, [picking_id], context) #PARA CUANDO DE EJECUTE EL PICKING Y PASE A ESTADO RESERVADO AUTOMATICAMENTE
    ...
    def action_button_confirm(self, cr, uid, ids, context=None):
        ...
        for o in self.browse(cr, uid, ids):
            c.update({'location': o.shop_id.warehouse_id.lot_stock_id.id })            
            if o.shop_id.warehouse_id.lot_stock_id.despacho_auto:                
                for line in o.order_line:
                    if line.product_id.type == 'product':
                        product = self.pool.get('product.product').browse(cr, uid, line.product_id.id, context=c)
                        if product.qty_disponible < line.product_uom_qty:
                            raise osv.except_osv(_('Warning!'),_('No puede vender un producto con stock insuficiente, Producto: "%s" (Cant. Dispo:%d).') %(line.product_id.name, product.qty_disponible,))
        wf_service = netsvc.LocalService('workflow')
