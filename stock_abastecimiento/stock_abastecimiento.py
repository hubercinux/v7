...
class stock_abastecimiento(orm.Model):
    ...
    _columns = {
        ...
        'user_id': fields.many2one('res.users', 'Usuario', select=True, track_visibility='onchange'),         
        'state': fields.selection([('draft','Nuevo'),('consol','Consolidado'),('progress','Pendiente'),('done','Abastecido'),('cancel','Anulado')],string='Estado',readonly=True,required=True),
        ...
        'consolidar':fields.boolean('Consolidar'),
        'cliente_id': fields.many2one('res.partner', 'Cliente', states={'done':[('readonly',True)], 'progress':[('readonly',True)], 'cancel':[('readonly',True)]}),
    ...
    def _prepare_picking(self, cr, uid, order, location_id, location_dest_id, context=None):
        ...
        journal_id= self.pool.get('stock.journal').search(cr, uid, [('warehouse_id','=',warehouse_id)])
        return {
    ...
    #Las siguientes lineas son para poder abastecimiento a PRINCIPAL
    def _create_picking_produccion(self,cr, uid, order, lines, picking_id=False, context=None):
        if context is None:
            context = {}
        ...
        location_dest_id = [79] #id de ubicacion de principal
        context.update({'location': location_id }) 
    ...
    def generar_guia_produccion(self, cr, uid, ids, context=None):
        for order in self.browse(cr, uid, ids, context=context):
            self._create_picking_produccion(cr, uid, order, order.abastec_line_ids, None, context=context)
            self.write(cr, uid, order.id, {'state': 'done'}, context)            
        return True

    #Evita eliminar un orden cuando tenga asignado un correlativo de venta.
    def unlink(self, cr, uid, ids, context=None):
        for abast in self.browse(cr, uid, ids, context=context):
            if abast.state not in ('cancel'):
                raise osv.except_osv(_('ERROR!'), _('Para eliminar debe primero, Anular este registro!'))
        return super(stock_abastecimiento, self).unlink(cr, uid, ids, context=context)  
      
class sale_order(osv.osv):
    _inherit = 'sale.order'
    def action_button_confirm(self, cr, uid, ids, context=None):
        action_confirmed = super(sale_order, self).action_button_confirm(cr, uid, ids, context=context)
        for order in self.browse(cr, uid, ids):
            #Genera order de abastecimiento desde PRODUCCION a PRINCIPAL
            self.generar_abastecimiento(cr, uid, order, order.order_line, None, None, context=context)            
        return True 

    def generar_abastecimiento(self, cr, uid, order, lines, abastec_prod_id=False, abastec_tall_id=False, context=None):
        if context is None:
            context = {}        
        abast_obj =  self.pool.get('stock.abastecimiento')
        abast_line_obj =  self.pool.get('stock.abastecimiento.line')
        for line in lines:
            partner_id  = [x.name.id for x in line.product_id.seller_ids]
            if line.product_id.type == 'product' and line.product_id.procedencia == 'nac' and line.product_id.supply_method=='produce':
                #PARA ABASTECIMIENTO DE PRODUCCION
                if partner_id[0] == 5710:#Id proveedor
                    warehouse_id = 6 #id deL ALMACEN PRODUCCION
                    if not abastec_prod_id:
                        abastec_prod_id = abast_obj.create(cr, uid, self._prepare_abastecimiento(cr, uid, order, warehouse_id, context), context)
                    move_id = abast_line_obj.create(cr, uid, self._prepare_abastecimiento_line_prod(cr, uid, line, abastec_prod_id, context))
                #PARA ABASTECIMIENTO DE TALLER
                if partner_id[0] == 5711: #Id proveedor
                    warehouse_id = 7 #id deL ALMACEN TALLER
                    if not abastec_tall_id:
                        abastec_tall_id = abast_obj.create(cr, uid, self._prepare_abastecimiento(cr, uid, order, warehouse_id, context), context)                 
                    move_id = abast_line_obj.create(cr, uid, self._prepare_abastecimiento_line_tall(cr, uid, line, abastec_tall_id, context))                    
        return True

    def _prepare_abastecimiento(self, cr, uid, order, warehouse_id, context=None):
        return {
            ...
            'user_id': uid ,
            'state': 'draft',
            'fecha_abast': order.fecha_expected or time.strftime('%Y-%m-%d %H:%M:%S'),
            'origin': order.name,
            'consolidar': False if order.fecha_expected  else True,
            'cliente_id': order.partner_id.id,
            'note': order.note or '',
        }    

    def _prepare_abastecimiento_line_prod(self, cr, uid, line, abastec_id, context=None):
        return {
            'product_id': line.product_id.id , 
    ...
    def _prepare_abastecimiento_line_tall(self, cr, uid, line, abastec_id, context=None):
        return {
            'product_id': line.product_id.id , 
            'product_qty': line.product_uom_qty, 
            'product_qty_abast': 0.0,
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        a_cancel =super(sale_order, self).action_cancel(cr, uid, ids, context=context)
        abast_obj =  self.pool.get('stock.abastecimiento')
        for order in self.browse(cr, uid, ids, context):
            uid=1
            abast_ids = abast_obj.search(cr, uid, [('origin','=',order.name)])
            for id in abast_ids:
              cr.execute("UPDATE stock_abastecimiento SET state='cancel' WHERE id=%s", (id,))
        return True
...
class pos_order(osv.osv):
    ...
    def create_picking(self, cr, uid, ids, context=None):
        picking = super(pos_order, self).create_picking(cr, uid, ids, context=context)
        for order in self.browse(cr, uid, ids):
            #Genera order de abastecimiento desde PRODUCCION a PRINCIPAL
            self.generar_abastecimiento(cr, uid, order, order.lines,None,context=context)
        return True
    ...
    def cancel_order(self, cr, uid, ids, context=None):
        a_cancel =super(pos_order, self).cancel_order(cr, uid, ids, context=context)
        abast_obj =  self.pool.get('stock.abastecimiento')
        for order in self.browse(cr, uid, ids, context):
            uid=1
            abast_ids = abast_obj.search(cr, uid, [('origin','=',order.name)])
...
class stock_picking(orm.Model):
    ...
    def draft_force_assign(self, cr, uid, ids , context=None, *args):
        res = super(stock_picking, self).draft_force_assign(cr, uid, ids, context=context, *args) 
        #_logger.error("INNNNNN111111: %r", ids)
        for pick in self.browse(cr, uid, ids):
            if pick.abastec_picking_id:
                for move in pick.move_lines:
                    abaste_line_id= self.pool.get('stock.abastecimiento.line').search(cr, uid, [('product_id','=',move.product_id.id),('abastec_id','=',pick.abastec_picking_id.id)])
                    if abaste_line_id:
                        self.pool.get('stock.abastecimiento.line').write(cr, uid, abaste_line_id, {'product_qty_abast': move.product_qty}, context)
        return True
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        for pick in self.browse(cr, uid, ids):
            if pick.abastec_picking_id:
                for move in pick.move_lines:
                    abaste_line_id= self.pool.get('stock.abastecimiento.line').search(cr, uid, [('product_id','=',move.product_id.id),('abastec_id','=',pick.abastec_picking_id.id)])
                    if abaste_line_id:
                        self.pool.get('stock.abastecimiento.line').write(cr, uid, abaste_line_id, {'product_qty_abast': 0.0}, context)
        return True

    def print_picking_abaste(self, cr, uid, ids, context=None):
        assert len(ids) == 1 
        datas = {
                 'model': 'stock.picking',
                 'ids': ids,
        ...
        return {'type': 'ir.actions.report.xml', 'report_name': 'stock.picking.composition', 'datas': datas, 'nodestroy': True}
...
class stock_move(orm.Model):
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            if move.picking_id.abastec_picking_id:
                abaste_line_id= self.pool.get('stock.abastecimiento.line').search(cr, uid, [('product_id','=',move.product_id.id),('abastec_id','=',move.picking_id.abastec_picking_id.id)])
                if abaste_line_id:
                    self.pool.get('stock.abastecimiento.line').write(cr, uid, abaste_line_id, {'product_qty_abast': 0.0}, context)
