...
class sale_shop(osv.osv):
    ...
    _columns = {
        ...
        #para reporte de documentos por tienda
        'shop_parent_id': fields.many2one('sale.shop', 'Tienda padre', select=True,help='Aque tienda principal pertecene'),
        'shop_email': fields.char('Correo de tienda',), 
    }
...
class pos_session(osv.osv):
    ...
    def _calc_total_caja(self, cr, uid, ids, field, args, context = None):
        res = {}
        for move in self.browse(cr, uid, ids, context = context):
            res[move.id] = {
                'total_caja': 0.0,
            }
            res[move.id]['total_caja'] = sum([x.total_entry_encoding for x in move.statement_ids])
        return res

    _columns = {    
        'total_caja': fields.function(_calc_total_caja,digits_compute=dp.get_precision('Account'), type='float', string = 'Total caja', multi='all'), 
    }
...
class pos_order(osv.osv):
    ...
    def _amount_subtotal(self, cr, uid, ids, name, args, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            #res[order.id]['amount_subtotal'] = cur_obj.round(cr, uid, cur, val2)
    ...
    _columns = {
        ...
        #'amount_subtotal': fields.function(_amount_subtotal, digits_compute=dp.get_precision('Point Of Sale'), string='Sub Total',multi='all',store=True),
        ...
        #agregado para nueva forma de impresion
        #datos de envio
        ...
        'delivery_agencia': fields.selection([('01','Si'),('02','No')],'Delivery agencia-domicilio',),
 
    }

    _defaults = {
        'es_factura': False,
        #'fecha_expected': lambda *a: time.strftime('%Y-%m-%d %H:%M:%S'),
    }
    ...
    def pos_print_report(self, cr, uid, ids, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            amount_payment = 0.0
            for value in order.statement_ids:
                amount_payment += value.amount
            if round(order.amount_total,2) != round(amount_payment,2):
                raise osv.except_osv(_('Warning!'),_('El Total cobrado, es diferente al total del ticket, Verifique!!!!'))
        datas = {
             'ids': ids,
             'model': 'pos.order',
...
class stock_picking(osv.osv):
    ...
    _columns = {
        #'pos_order_id':fields.many2one('pos.order','Orden venta ticket', states={'done':[('readonly', True)], 'assigned':[('readonly',True), 'confirmed':[('readonly',True)]}),        
        'pos_order_id':fields.many2one('pos.order','Orden venta ticket', ),        
        }
...
class pos_order_line(osv.osv):
    ...
    def _amount_line_all(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            #_logger.error("SHOP ID 0: %r", res)
        return res
    _columns = {#Añadimos precision para poder generar los asientos correctamente
        'descripcion': fields.char('Descripcion',),        
        'price_subtotal': fields.function(_amount_line_all, multi='pos_order_line_amount', string='Subtotal w/o Tax',digits_compute=dp.get_precision('Point Of Sale'),store=True),
