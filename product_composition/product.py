...
class stock_move(orm.Model):
    ...
    _columns = {
        ...
        'compuesto': fields.boolean('Compuesto'),
    }
...
class product_make(osv.osv):
    ...
    _columns = {
        ...
        'fecha_composicion': fields.boolean('Respetar fecha programada', help='Respetar fecha programada', states={'done':[('readonly',True)],'cancel':[('readonly',True)]}),        
    }
    _order='date desc'
    ...
    def _move_composition_line(self, cr, uid, composition, m, context=None):
        ...
        if m.concepto == 'producir':
            ...
            if composition.fecha_composicion:
                stock_move.write(cr, uid, [move_id],{'date': composition.date}, context)
...
class sale_order(osv.osv):
    ...
    def _prepare_order_line_move(self, cr, uid, order, line, picking_id, date_planned, context=None):
        ...
        #_logger.error("MINIMO222222: %r", res)
        comp_ids = self.pool.get('product.composition').search(cr, uid, [('product_id','=',line.product_id.id)])
        if comp_ids:
            res['compuesto'] = True
        return res
            ...
            if order.almacendespacho_id:
                ...
                for line in order.lines:
                    if line.product_id and line.product_id.type == 'service':
                        continue
                    comp_ids = self.pool.get('product.composition').search(cr, uid, [('product_id','=',line.product_id.id)])#PARA PRODUCTO COMPUESTOS
                    move_obj.create(cr, uid, {
                        ...
                        'compuesto': True if comp_ids else False,#PARA PRODUCTO COMPUESTOS
                    }, context=context)
